...
export const Hero: React.FC = () => {
  ...
  // Canvas dust particles — pure JS, no GSAP needed
  useEffect(() => {
  ...
  // CSS float keyframes — each pill uses a different delay via inline animationDelay
  const floatStyle = (i: number): React.CSSProperties => ({
    animation: `heroFloat ${3.5 + i * 0.4}s ease-in-out ${i * 0.35}s infinite alternate`,
  });

  return (
    <section
      id="home"
      className="relative w-full h-screen min-h-[760px]"
      style={{ overflow: 'clip' }}
    ...
    >
      ...
      <div className="relative z-10 w-full h-full flex flex-col md:flex-row items-center max-w-7xl mx-auto px-6 md:px-14 pt-24">
        ...
        <div className="w-full md:w-[55%] flex flex-col justify-center text-left order-2 md:order-1 mt-8 md:mt-0">
          ...
          {/* Headlines — Framer Motion stagger, no GSAP */}
          <div className="mb-6" style={{ overflow: 'clip' }}>
            {[
              { text: 'Handmade', className: 'text-[#FFF8EE]', italic: false },
              { text: 'Tradition.', gradient: true, italic: true },
              { text: 'Crafted With Love.', className: 'text-[#FFF8EE]/70 text-[2rem] sm:text-[2.5rem] lg:text-[3rem]', small: true },
            ].map((line, i) => (
              <motion.h1
                key={i}
                ...
                className={`font-display leading-[1.05] ${line.small ? '' : 'text-[3.6rem] sm:text-[4.8rem] lg:text-[5.8rem]'} ${line.italic ? 'italic' : ''} ${line.className ?? ''}`}
                style={line.gradient ? {
                  background: 'linear-gradient(90deg,#D9A520,#F5C842,#C7852B)',
          ...
          >
            <a href="#products" className="font-body px-8 py-3.5 bg-[#D9A520] text-[#3B2314] rounded-full font-semibold text-sm tracking-wide transition-all duration-300 hover:bg-[#F5C842] hover:scale-105 hover:shadow-[0_0_30px_rgba(217,165,32,0.55)]">
              Explore Snacks
            </a>
            <a href="#process" className="font-body px-8 py-3.5 border border-[#D9A520]/50 text-[#FFF8EE] rounded-full font-medium text-sm tracking-wide transition-all duration-300 hover:bg-[#D9A520]/10 hover:border-[#D9A520]">
              Watch Process
            </a>
            <a href="#bulk" className="font-body text-[#D9A520] font-medium text-sm flex items-center gap-1.5 group ml-1 transition-all duration-300">
        ...
        <div className="w-full md:w-[45%] h-[46vh] md:h-full relative flex items-center justify-center order-1 md:order-2">
          ...
          >
            {ingredients.map((ing, i) => (
              <div
                key={i}
                className="absolute px-4 py-2 rounded-full text-[#FFF8EE]/85 text-xs font-body font-medium tracking-wide whitespace-nowrap"
                style={{
                  top: ing.top, left: (ing as any).left, right: (ing as any).right, bottom: (ing as any).bottom,
          ...
          {/* Murukku SVG — CSS spin, CSS parallax via mouse state */}
          <svg
            className="w-[240px] h-[240px] sm:w-[350px] sm:h-[350px] lg:w-[420px] lg:h-[420px] drop-shadow-[0_24px_48px_rgba(0,0,0,0.6)] transition-transform duration-700 ease-out"
            style={{
      ...
      {/* Scroll indicator — CSS bounce, no Framer needed */}
